 ...
 */
public class ReplicaLeaderSizeInCost implements HasMoveCost {
  ...
  public MoveCost moveCost(ClusterInfo before, ClusterInfo after, ClusterBean clusterBean) {
    ...
    var cost =
        changePartitions.stream()
            .flatMap(p -> after.replicas(p).stream().filter(r -> !before.replicas(p).contains(r)))
            .collect(
    ...
    return MoveCost.movedReplicaLeaderInSize(result);
  }
