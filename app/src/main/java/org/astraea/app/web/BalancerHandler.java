...
import org.astraea.common.cost.PartitionMigrateTimeCost;
import org.astraea.common.cost.RecordSizeCost;
import org.astraea.common.cost.ReplicaLeaderCost;
import org.astraea.common.cost.ReplicaLeaderSizeInCost;
import org.astraea.common.cost.ReplicaLeaderSizeOutCost;
...
class BalancerHandler implements Handler {
  ...
  private static List<MigrationCost> migrationCosts(MoveCost cost) {
    return Stream.of(
            ...
            new MigrationCost(
                MOVED_IN_SIZE,
                cost.movedReplicaLeaderInSize().entrySet().stream()
                    .collect(
                        Collectors.toMap(
                            e -> String.valueOf(e.getKey()), e -> (double) e.getValue().bytes()))),
            new MigrationCost(
                MOVED_OUT_SIZE,
                cost.movedReplicaLeaderOutSize().entrySet().stream()
                    .collect(
                        Collectors.toMap(
                            e -> String.valueOf(e.getKey()), e -> (double) e.getValue().bytes()))),
            new MigrationCost(
